...
namespace Boltzmann_distribution
{
    internal class MyVector
    {
        ...
        public MyVector GetNormal() => new MyVector(-Y, X); 

        public float LengthqSuared() => X * X + Y * Y;
        public float Length() => (float)Math.Sqrt(LengthqSuared());
        ...
        public static int signCos(MyVector v1, MyVector v2) => Math.Sign(dot(v1, v2));
        public static int signSin(MyVector v1, MyVector v2) => Math.Sign(mult_coorZ(v1, v2));
        public static float Cos(MyVector v1, MyVector v2) => dot(v1, v2) / (v1.Length() * v2.Length());
        public static float Sin(MyVector v1, MyVector v2) => mult_coorZ(v1, v2) / (v1.Length() * v2.Length());
